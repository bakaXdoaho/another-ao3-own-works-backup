...
RESUME_MAX_ROUNDS = 6         # 最多自动续几轮（0 = 不自动续，跑一轮就退出）
...
# ⚠️ **抄这个数字的日期**。必须跟着上面那行一起改。
#
# 为什么要记日期：这是**唯一一个靠人手抄进来的参照值**（AO3 只在 stats 页给串数，
# 别处拿不到）。数字本身会随新评论增长，所以它迟早会跟本地库对不上 ——
# 那时候看到「差 +2」，需要一眼能分清是**数据出问题**还是**参照值过期了**。
# 不记日期就分不清，于是每轮都要重新查一遍同一件事。
#
# 一条通用的规矩：**任何「从外部抄进来的参照值」都该带上抄它的日期**，
# 否则日后对不上时，没人分得清是数据变了还是参照值旧了。补上。
AO3_STATS_COMMENT_THREADS_READ_ON = ""
